...
{	int rc = 0, rc1=0,  l, ln;
	...
	l = (nel>>8); //всего элементов лога, если 255, то может быть и больше. Было до отправки пакета. После отправки - меньше на ln
	if(l > ln && rc1 == 0)
	{ tmp2 = l - ln;
...
//			if(tmp > 15) tmp = 15;
		...
		case 0x10: //подтверждение + начальные значения PID после рестарта
...
/* два раза проверяем, установилась ли уставка, потом сбрасываем признак наличия команды. Чтобы из веб интрефейса и HA можно было задать новую уставку */
	if(pSmt->isCmd == 2)
